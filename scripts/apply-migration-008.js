// One-shot migration 008 application + verification for the local DB.
...
const sql = fs.readFileSync(path.join('migrations', '008_drop_milestone_uniqueness.sql'), 'utf-8');

const already = db.prepare(`SELECT 1 FROM _migrations WHERE name = ?`).get('008_drop_milestone_uniqueness.sql');
if (already) {
  console.log('\n[skip] migration 008 already applied');
} else {
  db.transaction(() => {
    db.exec(sql);
    db.prepare('INSERT INTO _migrations (name, applied_at) VALUES (?, ?)').run(
      '008_drop_milestone_uniqueness.sql',
      new Date().toISOString(),
    );
  })();
  console.log('\n[apply] migration 008 applied');
}
...
if (!sampleRow) {
  ...
  try {
    ...
    db.prepare(`DELETE FROM subscriber_snapshots WHERE polled_at = ? AND source = 'test_008'`).run(testPolledAt);
