...
function RisingChannelsPanel({ channels }: Pick<BottomPanelsProps, 'channels'>) {
  ...
  //
  // liveSurge = 보간된 현재값 - (서버 폴 시점의 값 - 서버 surgeDelta24h)
  //           = surgeDelta24h + (interpolated - polled)
  // 보간 드리프트와 함께 매 프레임 갱신돼서, 30초 폴 사이에도 숫자가 살아 움직임.
  // 정렬도 liveSurge 기준이라 표시값과 순서가 항상 일치.
  const rising = channels
    .filter((c) => c.surgeDelta24h != null && c.surgeDelta24h > 0)
    .map((c) => ({
      channel: c,
      liveSurge: (c.surgeDelta24h ?? 0) + (c.subscriberCount - c.polledSubscriberCount),
    }))
    .sort((a, b) => b.liveSurge - a.liveSurge)
    .slice(0, 5);
  ...
  return (
    ...
    >
      ...
      {rising.length === 0 ? (
        ...
        <LayoutGroup id="rising-channels">
          <div className="flex-1 grid gap-[6px]" style={{ gridTemplateColumns: 'repeat(5, minmax(0, 1fr))' }}>
            {rising.map(({ channel, liveSurge }, index) => (
              <motion.div
                key={channel.id}
                layoutId={`rising-${channel.id}`}
                layout="position"
              ...
              >
                ...
                <div className="m-0 min-w-0 flex-1">
                  ...
                  <p className="m-0 mt-[2px] text-[11px] font-[850]" style={{ color: 'var(--color-increase)', fontVariantNumeric: 'tabular-nums' }}>
                    +{liveSurge.toLocaleString('ja-JP')}
                  </p>
                </div>
              </motion.div>
            ))}
